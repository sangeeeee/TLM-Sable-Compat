...
结构内钓鱼及曲柄交互站位的规则与修复见 [钓鱼与曲柄任务修复](docs/fishing-crank-fixes.md)。女仆钓鱼仍需乘坐椅子或船。

地面导航卡住时会低频尝试安全的局部绕行；森罗厨房菜板任务中断后可继续加工半成品。适用范围和测试见 [局部避障与菜板中断恢复](docs/local-avoidance-and-board-resume.md)。

支持 Maid Tavern 的葡萄收获与酿酒流程，包括箱子取料、桶盖投料、装瓶收瓶和成品入库，遵守 Home 与河童罗盘范围。规则与测试见 [女仆酒馆兼容](docs/maid-tavern-compatibility.md)。
...
森罗厨房、酒馆、女仆摇曲柄、节气及其多维修补、女仆厨房、农夫乐事、烧烤乐事，以及女仆酒馆、女仆餐厅、女仆实用任务、女仆管理与存储类附属已添加为仅编译依赖，版本及前置见 [附属任务编译依赖](docs/compile-dependencies.md)。
